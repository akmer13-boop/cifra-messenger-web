...
test("projects latest Tinode activity, preview, time and unread count into the chat row", async () => {
...
test("clears realtime unread state when the chat is opened or visible", async () => {
  const page = await readFile(pageUrl, "utf8");

  assert.match(page, /id !== realtimeObservedTopic \|\| !realtimeUserId/);
